...
| Item | Status | Notes |
|---|---|---|
| `_shared/tokens.css` | ✅ | **Djera design system** (ported from `claude-design-mockups`). Warm Libyan / North-African identity in **oklch**: palm-green brand, saffron accent, terracotta, warm cream surfaces, warm-midnight dark theme. Three palette variants via `[data-palette]` (palm default · saffron · date). Fonts: Geist (display) / Inter (sans) / Geist Mono / IBM Plex Sans Arabic (RTL) via Google Fonts `@import`. Refined radii (`--r-xs…--r-2xl`), shadows, easings. Back-compat aliases keep older pages working (`--radius`→`--r-md`, `--shadow`→`--shadow-md`). |
...
| `_shared/app.css` | ✅ | **Djera component layer** (ported from `claude-design-mockups`). iPhone 17 Pro Max frame (440 × 956), Dynamic Island, grain-textured stage, status bar, home indicator. Primitives: `.btn`(primary/secondary/ghost/danger), `.field`, `.card`, `.badge`(brand/warn/danger/mute + legacy success/warning aliases), `.segment`, `.row`, `.appbar`, `.navbar`/`.nav-item`, `.fab`, `.todo-chip`, type scale (`.h1/.h2/.h3/.display/.eyebrow`), animations. Re-added legacy primitives (`.brand-mark`, `.title`, `.subtitle`, `.section-label`, `.upload`) restyled to the new tokens. Driver additions kept: `.screen > *` flex-shrink, `.has-navbar` bottom spacing, collapsible toolbar. Logical properties → RTL-safe. |
| `_shared/i18n.js` | ✅ | `I18N.register` / `setLang` / `setTheme` / `apply`. EN/AR + light/dark, persisted to localStorage. |
| `_shared/frame.js` | ✅ | Injects status bar (9:41 + signal/wifi/battery SVGs) and home indicator into every `.phone`. Also computes `--phone-scale` from `innerWidth/innerHeight` so the frame auto-fits any viewport. Adds a **collapse handle** to the dev toolbar (sliders icon) — collapsed by default, click to reveal EN/AR/theme/back; state persists in `localStorage` (`jeera.toolbar`). |
...
| Root `index.html` | ✅ | Redirects to `welcome/`. Also points to `dev.html` (the dev hub) as a fallback link. |
| `dev.html` | ✅ | **Dev hub.** Desktop-friendly screen index for QA — every feature grouped by phase (D1–D4) as a card with description + per-screen mono chips that deep-link into the phone. Live search filters cards across name/folder/description/screen labels. Header carries Lang (EN/عربي) + Theme (Light/Dark) toggles that write to the same `jeera.lang`/`jeera.theme` localStorage keys `_shared/i18n.js` uses, so flipping here persists into every screen you open. Stats strip (features · screens · phases · status). Built on `_shared/tokens.css` only — no phone frame, no i18n.js, no map.js. Footer links to `TRACKING.md`, monorepo `CLAUDE.md`, and the playbook. |
...
| Folder | Screens | Status | Spec | Notes |
|---|---|---|---|---|
| `dashboard/` | `index.html` | ✅ | §2.3 | **Full-bleed map home** (real ride-hailing pattern). Leaflet+OSM map of Tripoli fills the whole phone (`map-fill`, `isolation:isolate` so Leaflet panes stay below the status bar); driver GPS-chevron pin centred, gains a pulse ring when online. **Matches `claude-design-mockups` ScreenDashboard (pill variant).** Floating overlays: avatar fab (brand "AM" → `profile/`) top-start · **online toggle pill** top-centre (Go online ↔ You're online; off = surface, on = brand-600 with white pulsing dot + glow; pulses the driver pin and mocks an incoming request 3s later → `ride-requests/`) · notifications bell (accent dot) top-end · stacked map-control fabs (layers, recenter) below it. Bottom **dock** (two cells, eyebrow labels): today's earnings (→ `earnings/`) · outstanding commission §2.10 in saffron with chevron (→ `commission/`); bottom **dock** with two cells — today's earnings (→ `earnings/`) and outstanding commission §2.10 (→ `commission/`). **Bottom tab bar** via `_shared/navbar.js` (Home active). Currency = `LYD / د.ل`. |
...
| `settings/` | `index.html` | ✅ | §3 | **Settings** (Djera system). Appbar; **Appearance** — Language (EN/عربي) + Theme (Light/Dark) segmented, **functional** (drive `I18N.setLang`/`setTheme`, persisted). **Notifications** toggles (trip requests · earnings · promotions). **Security** — App lock (PIN) + biometric toggles + TODO §5 chip. **Privacy** — privacy & terms · help & support (→ `support/`) · sign out. Reached from Profile (gear + "Language & theme" row). EN/AR + light/dark. |
| `support/` | `index.html` | ✅ | §3 | **Help & support** (Djera system). Appbar; prominent **SOS** card (danger gradient → alert emergency services + safety team); **Contact** list (call support `tel:` · live chat · email `mailto:`); **FAQ accordion** (5 Q&A — pay, commission, no-show, documents, going offline — tap to expand). Linked from Profile → "Help & support" and from `trip-history/detail.html`. EN/AR + light/dark. |
