...
| Folder | Screens | Status | Spec | Notes |
|---|---|---|---|---|
| `dashboard/` | `index.html` | ✅ | §2.3 | **Map-first layout** (per Uber/Careem driver pattern). Compact header (avatar `AM` + name + "Jeera driver" + hamburger). **Real Leaflet+OSM map of Tripoli** via `_shared/map.js`, driver bike-pin in the centre (gains pulse ring when online), overlaid status pill (top-start) and today's-earnings chip (top-end, tap → trip-history). Online toggle row with iOS-style switch syncs the pill + pin pulse + primary CTA + status line. Outstanding-commission card per §2.10 → links to `commission/`. Primary CTA mirrors the toggle (`Start ↔ Stop receiving requests`); secondary → `trip-history/`. Going online mocks an incoming request 3.5s later and routes to `ride-requests/`. Currency = `LYD / د.ل` via shared `unit.currency` i18n key. |
